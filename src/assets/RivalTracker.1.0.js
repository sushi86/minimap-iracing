...
var RivalTracker = (function () {
	...
	var fastestUpdateRateSupported = 100; // drop fast incoming updates
        ...
        this.getDriverPosition = function(percent) {
            return getPointAt(percent);
        }

        this.getAngle = function(percent) {
            var point = getPointAt(percent - 0.01);
            var angle = getPointAt(percent + 0.01);

            var dx =  point.x - angle.x;
            var dy =  point.y - angle.y;
            var dAngle = Math.atan(dy/dx) * (180 / Math.PI);
